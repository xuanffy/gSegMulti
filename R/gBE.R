...
#' @description This function prunes candidate change-points and choose the model with the largest exteded pseudo-BIC values.
...
#' @param graph Type of similarity graphs.
#'
...
gBE <- function(y,
  ...
  gofoverall[1] <- sum(gofstat) - c * (length(tauhat) - 2) * log(n)
  ...
  while (length(tauhat) >= (minCpNum + 4) &&
    ...
    gofoverall[i + 1] <- sum(gofstat) - c * (length(tauhat) - 2) * log(n)
    i <- i + 1
  ...
  if (length(tauhat) >= (minCpNum + 3) && n < maxSeq) {
    ...
    gofoverall[i + 1] <- gofstat - c * log(n)
  }
